...
On the surface level, this did solve one of the problems with the earlier contests: there was a non-trivial amount of luck involved in each submission. For a deterministic strategy some words are "easier" to guess than others, so your score depended to some degree on which words you received. For example, in the first competition one of the other contestants ran a simulation to determine the expected number of times they would need to submit to beat my submission.

put the picture of the guy trying to brute force it here

*It was a pleasant surprise to see such a direct result of Central Limit Theorem emerge outside of a stats class*

However, there was a different problem with the new contest format: the optimal five-letter Wordle strategy was already an open secret -- [the list of all the Wordle optimality results so far](https://www.poirrier.ca/notes/wordle-optimal/) had already been published at this point. Anyone could look up the decision tree of one of the strategies in those lists and compete with that, which would net a consistent score of 7920.

Well great, just get a 7920 and you win! Not quite. 7920 is the best possible score if you use the same decision tree for all of your guesses, i.e. always use the same first guess for every word, receive feedback, always use the same second guess for that feedback. Could there be an advantage to using a mixed strategy?

In the first three contests, there was no advantage to using a mixed strategy -- if the best strategy can solve a word in 3.421 guesses on average, mixing it with the second best strategy that gets 3.422 words on average would only make the expected value of your score worse. The fourth contest had a subtle, but huge difference: secret words were sampled *without* replacement -- as soon as we identify a word we can remove it from the pool of possibilities for any unresolved targets. When using a single strategy this information is totally useless, since the turn we receive feedback that uniquely identifies a word is the same turn we receive feedback for other words that disambiguates them from the solved word anyway. With multiple strategies, there's a chance that one strategy can find information for another while its still actionable.
...
* What's mix of two strategies (nets lowest expected number of guesses): I imagine this is harder than just mixing the two best strategies -- some strategy pairs might have better "synergy", i.e. reveal information that others might find "useful" more frequently. For what it's worth, I'm pretty sure computing this exactly is extremely intractable, but I'm frequently wrong about this sort of thing.
* What's the optimal mix of any number of strategies? (Probably super intractable)
* My last submission only mixed strategies during the first turn. Can we do better by mixing strategies at the other turns as well?
